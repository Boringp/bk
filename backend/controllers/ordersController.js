...
const createOrder = async (req, res, next) => {
    const { orderNumber, shippingAddress, billingAddress,items,total } = req.body;
...
const getOrderByNumber = async (req, res, next) => {
  ...
  if (!order) {
    const error = new HttpError(
      'Could not find an order with the provided order number.',
      404
    );
    return next(error);
  }
...
const getOrdersForUser = async (req, res, next) => {
  ...
  res.json({ orders: userOrders.map(order => order.toObject({ getters: true })) });
};
